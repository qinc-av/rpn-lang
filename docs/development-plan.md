...
| SwiftPM package | `Package.swift` exports two products: `RpnLangCXX` (the C++ sources, C++20, public headers under `include/`) and `RpnLang` (the Swift wrapper `swift/RpnLang.swift`, built with `.interoperabilityMode(.Cxx)` — it `@_exported import`s `RpnLangCXX` and adds an async/await `eval`). Swift sees only the narrow `rpn-hl.h` surface via `include/module.modulemap`; `include/` also carries symlinks (`rpn.h`, `rpn-matrix.h`, `vecmx3.h`, `nlohmann/`, `Eigen/`) so C++ consumers such as `rpn-color` pick up the broader headers through SwiftPM's `publicHeadersPath`. RP42 consumes the package directly — no ObjC bridge, no `.mm`. |
