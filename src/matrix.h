...
public:
   ...
   matrixT& operator = (const T &v) {
     if (_m->Refcnt > 1) clone();
     for(unsigned r=0; r<_m->Row; r++)
       for(unsigned c=0; c<_m->Col; c++)
	 _m->Val[r][c] = v;
     return *this;
   };
   ...
   void Null () _NO_THROW;
   void Identity (const size_t& row) _NO_THROW;
   void Identity () _NO_THROW;
   void SetSize (size_t row, size_t col) _NO_THROW;

   // row column extraction
   matrixT Row(size_t row) const _THROW_MATRIX_ERROR;
   matrixT Col(size_t col) const _THROW_MATRIX_ERROR;
   matrixT Cols(size_t *cols, size_t n) const _THROW_MATRIX_ERROR;
   matrixT Crop(size_t r0, size_t c0, size_t r1, size_t c1) const _THROW_MATRIX_ERROR;
   matrixT Lag(int l) const;
   matrixT Diff(int l) const;
   matrixT Stats() const;
   ...
   T Cond () _NO_THROW;
   matrixT Diag() const _THROW_MATRIX_ERROR;
   matrixT Sqrt() const _NO_THROW;
   matrixT map(double (*fn)(double)) _NO_THROW;
   ...
   void WriteCsv(std::ostream &os, const char **colnames=0) const;
   void WriteCsv(const char *fn, const char **colnames=0) const;
private:
    struct base_mat
    {
	T **Val;
...
matrixT::realloc (size_t row, size_t col)
{
   ...
   size_t colSize = std::min(_m->Col,col) * sizeof(T);
   size_t minRow = std::min(_m->Row,row);
...
matrixT::Row(size_t row) const _THROW_MATRIX_ERROR {
  if (row >= _m->Row)
    REPORT_ERROR( "matrixT::Col(): Index out of range!");
  matrixT temp(1, _m->Col);
  for (size_t j=0; j<_m->Col; j++)
    temp._m->Val[0][j] = _m->Val[row][j];
  return temp;
}
...
matrixT::Col(size_t col) const _THROW_MATRIX_ERROR {
  ...
  matrixT temp(_m->Row, 1);
  for (size_t i=0; i<_m->Row; i++)
    temp._m->Val[i][0] = _m->Val[i][col];
  return temp;
}
...
matrixT::Cols(size_t *cols, size_t n) const _THROW_MATRIX_ERROR {
  for(unsigned i=0; i<n; i++) {
    if (cols[i] >= _m->Col) REPORT_ERROR( "matrixT::Col(): Index out of range!");
  }

  matrixT temp(_m->Row, n);
  for (size_t i=0; i<_m->Row; i++)
    for(unsigned j=0; j<n; j++) {
      temp._m->Val[i][j] = _m->Val[i][cols[j]];
    }
  return temp;
}
...
MAT_TEMPLATE matrixT
matrixT::Crop(size_t r0, size_t c0, size_t r1, size_t c1) const _THROW_MATRIX_ERROR {
  if (r0 >= _m->Row || c0 >= _m->Col ||
      r1 >= _m->Row || c1 >= _m->Col ||
      r0>r1 || c0>c1)
    REPORT_ERROR( "matrixT::Crop(): Index out of range!");
  ...
  for (size_t i=0; i<nr; i++)
    for (size_t j=0; j<nc; j++)
      temp._m->Val[i][j] = _m->Val[r0+i][c0+j];
  return temp;
}
...
matrixT::Lag(int l) const {
  ...
  for(int r=0; r<_m->Row; r++)
    for(int c=0; c<_m->Col; c++) {
      temp._m->Val[r][c] = (((r-l)<0) || (r-l)>=_m->Row) ? T(NAN) : _m->Val[r-l][c];
    }
  return temp;
}
...
matrixT::Diff(int l) const {
  ...
  for(int r=0; r<_m->Row; r++)
    for(int c=0; c<_m->Col; c++) {
      temp._m->Val[r][c] = (((r-l)<0) || (r-l)>=_m->Row) ? T(NAN) : _m->Val[r][c] - _m->Val[r-l][c];
    }
  return temp;
}
...
MAT_TEMPLATE matrixT
matrixT::Stats() const {
  matrixT temp(5, _m->Col);
  for(int c=0; c<_m->Col; c++) {
    temp._m->Val[0][c] = std::numeric_limits<T>::max(); // min
    temp._m->Val[1][c] = -std::numeric_limits<T>::max(); // max
    temp._m->Val[2][c] = 0; // range
    temp._m->Val[3][c] = 0; // mean
    temp._m->Val[4][c] = 0; // variance
  }

  // pass 1: min, max, mean
  for(int r=0; r<_m->Row; r++)
    for(int c=0; c<_m->Col; c++) {
      temp._m->Val[0][c] = std::min(temp._m->Val[0][c], _m->Val[r][c]);
      temp._m->Val[1][c] = std::max(temp._m->Val[1][c], _m->Val[r][c]);
      temp._m->Val[3][c] += _m->Val[r][c];
    }

  // pass 2: variance sum
  for(int r=0; r<_m->Row; r++)
    for(int c=0; c<_m->Col; c++) {
      double err = _m->Val[r][c] - temp._m->Val[3][c];
      temp._m->Val[4][c] += (err*err);
    }

  // now final calculations on each column
  for(int c=0; c<_m->Col; c++) {
    temp._m->Val[2][c] = temp._m->Val[1][c] - temp._m->Val[0][c];
    temp._m->Val[4][c] /= T(_m->Row-1);
  }
...
{
  ostrm << "Matrix[" << m.RowNo() << "x" << m.ColNo() << "]"<<std::endl;
  ostrm << "[ ";
   for (size_t i=0; i < m.RowNo(); i++)
   {
     ostrm << "[ ";
...
MAT_TEMPLATE inline matrixT&
matrixT::operator += (const T &v)  _NO_THROW {
  if (_m->Refcnt > 1) clone();
...
// binary subtraction operator
MAT_TEMPLATE inline matrixT
operator - (const matrixT& m1, const T& c) _THROW_MATRIX_ERROR
{
   matrixT temp = m1;
   temp -= c;
   return temp;
...
MAT_TEMPLATE inline void
matrixT::Identity (const size_t& row) _NO_THROW
{
    if (_m->Refcnt > 1) 
	clone();
...
// return the diagnonal as an row vector (nx1)
MAT_TEMPLATE inline matrixT
matrixT::Diag () const _THROW_MATRIX_ERROR
{
  if (_m->Row != _m->Col)
    REPORT_ERROR( "matrixT::Diag(): non-square matrix!");
  matrixT d(_m->Row, 1);
  for(size_t i=0; i < _m->Row; i++)
    d._m->Val[i][0] = _m->Val[i][i];
  return d;
}

MAT_TEMPLATE inline matrixT
matrixT::map(double (*fn)(double)) _NO_THROW {
  matrixT r(_m->Row, _m->Col);
  for(size_t i=0; i < _m->Row; i++)
    for(size_t j=0; i < _m->Col; i++)
      r._m->Val[i][j] = fn(_m->Val[i][j]);
  return r;
}

// return the sqrt of each element
MAT_TEMPLATE inline matrixT
matrixT::Sqrt () const _NO_THROW
{
  matrixT r(_m->Row, _m->Col);
...
MAT_TEMPLATE void
matrixT::WriteCsv(const char *fn, const char **colnames) const {
  std::ofstream os;
  os.open(fn);
  WriteCsv(os, colnames);
  os.close();
}

MAT_TEMPLATE void
matrixT::WriteCsv(std::ostream &os, const char **colnames) const {
  if (colnames) {
    for (size_t j=0; j < _m->Col-1; j++) {
      os << colnames[j] << ", ";
    }
    os << colnames[_m->Col-1] << std::endl;
  }
  ...
  for (size_t i=0; i < _m->Row; i++) {
    ...
    os <<  _m->Val[i][_m->Col-1] << std::endl;
  }
}

MAT_TEMPLATE int
max(matrixT &mx, int col) {
  T l=-std::numeric_limits<T>::max();
  int mxx=0;
  for(unsigned i=0; i<mx.RowNo(); i++) {
    if (mx(i,col)>=l) {
      l=mx(i,col);
      mxx=i;
    }
